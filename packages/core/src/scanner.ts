...
class Scanner implements MykadScanner {
  ...
  private tick(): void {
    ...
    if (t.cardPresent && t.sharp) {
      // Hand tremor pauses the hold instead of restarting it; only losing
      // the card (or focus) resets. This keeps 0.1.0's false-positive
      // resistance: reaching the lock still requires the card to stay
      // detected the whole way through.
